...
class Lesson extends \yii\db\ActiveRecord
{
    ...
    public function isOwing($enrolmentId)
    {
        ...
        return (round($this->getCreditAppliedAmount($enrolmentId), 2) - round($this->isPrivate() ? $this->privateLesson->total  : $this->getGroupNetPrice($enrolment), 2)) < -0.09;
    }
    ...
    public function beforeSave($insert)
    {
        ...
        if (!$this->teacherRateOld) {
            $this->teacherRateOld = 0.0;
        }
    ...
    public static function triggerPusher()
    {
        $options = [
        ...
        return $pusher->trigger('lesson', 'lesson-edit', '');
    }
    ...
    public function calcLessonPrice() 
    {
       $course = Course::findOne($this->course->id);
        ...
        $lastLessonPrice = $rate - (round($rate / $lessonsCount, 2) * ($lessonsCount - 1));
        $pricePerLesson = round($rate / $lessonsCount, 2);
