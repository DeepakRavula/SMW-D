...
class TimelineEventPayment extends \yii\db\ActiveRecord
{
    ...
    public function editPayment($event)
    {
        ...
        $timelineEvent = Yii::$app->commandBus->handle(new AddToTimelineCommand([
            'data' => $payment,
           'message' => $paymentModel->userName . ' changed  '.$paymentModel->paymentMethod->name.'  payment  amount  from  '.Yii::$app->formatter->asCurrency($data['amount']) . ' to ' . Yii::$app->formatter->asCurrency($paymentModel->amount).'  on    {{invoice #' . $paymentModel->invoice->getInvoiceNumber() . '}}',
        ]));
        if ($timelineEvent) {
            $timelineEventLink = new TimelineEventLink();
            $timelineEventLink->timelineEventId = $timelineEvent->id;
