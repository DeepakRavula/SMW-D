...
$js = '
jQuery(".dynamicform_group_qualification").on("afterInsert", function(e, item) {
    jQuery(".dynamicform_group_qualification .panel-title-group-qualification").each(function(index) {
        jQuery(this).html("Qualification: " + (index + 1))
    });
});

jQuery(".dynamicform_group_qualification").on("afterDelete", function(e) {
    jQuery(".dynamicform_group_qualification .panel-title-group-qualification").each(function(index) {
        jQuery(this).html("Qualification: " + (index + 1))
    });
});
...
DynamicFormWidget::begin([
	'widgetContainer' => 'dynamicform_group_qualification', // required: only alphanumeric characters plus "_" [A-Za-z0-9_]
	...
	'model' => $groupQualificationModels[0],
	'formId' => 'dynamic-form',
	'formFields' => [
		'program_id',
		'rate',
...
<div class="row-fluid">
	...
	<div class="group-qualification-container-items group-qualification-fields form-well">
		<?php foreach ($groupQualificationModels as $index => $groupQualificationModel): ?>
			<div class="item-block group-qualification-item"><!-- widgetBody -->
				<h4>
					<span class="panel-title-group-qualification pull-left">Qualification: <?= ($index + 1) ?></span>
					<button type="button" class="pull-right group-qualification-remove-item btn btn-danger btn-xs"><i class="fa fa-remove"></i></button>
					<div class="clearfix"></div>
				</h4>
				<?php
				if (!$groupQualificationModel->isNewRecord) {
					echo Html::activeHiddenInput($groupQualificationModel, "[{$index}]id");
				}
				$groupPrograms = ArrayHelper::map(Program::find()->group()->all(), 'id', 'name');
				?>
				<div class="row">
					<div class="col-sm-4">
						<?= $form->field($groupQualificationModel, "[{$index}]program_id")->dropDownList($groupPrograms);?>
					</div>
					<div class="col-sm-4">
	<?= $form->field($groupQualificationModel, "[{$index}]rate")->textInput(['maxlength' => true]) ?>
					</div>
					<div class="clearfix"></div>
				</div>
			</div><!-- widgetBody -->
