...
<?php
    ...
    array_push($columns, [
        ...
        'label' => 'Balance',
        'value' => function ($data) {
            return !empty($data->balance) ? (round($data->balance, 2) > 0.00 && round($data->balance, 2) <= 0.09) || (round($data->balance, 2) < 0.00 && round($data->balance, 2) >= -0.09) ? Yii::$app->formatter->asCurrency(round($data->balance, 2)) : Yii::$app->formatter->asCurrency(round(0.00, 2)) : null;
        }
    ]);
