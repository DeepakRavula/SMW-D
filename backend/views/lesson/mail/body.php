...
<?php if (!empty($model->reschedule) && !empty($model->enrolment) && !$model->bulkRescheduleLesson) : ?>
    ...
    <?php if (!empty($model->date) && new \DateTime($model->date) != new \DateTime($lesson->date)) : ?>
        <?= $studentName . '\'s ' . $programName . ' lesson  on ' . $oldLessonDate . ' @ ' . $oldLessonTime . ' for ' . $oldLessonDuration . ' minutes has been rescheduled to ' . $lessonDate . ' @ ' . $lessonTime . ' for ' . $lessonDuration . ' minutes with ' . $newTeacherName . "."; ?>
